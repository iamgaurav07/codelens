...
export async function sendHighSeverityAlert({
  ...
  await transporter.sendMail({
    from: `CodeLens <${process.env.EMAIL_FROM}>`,
    to,
    ...
    html: `
      <div style="font-family: -apple-system, sans-serif; max-width: 560px; margin: 0 auto; padding: 40px 20px; background: #0A0A0F; color: #fff;">
        ...
        <div style="display: flex; gap: 12px;">
          <a href="${reviewUrl}" style="background: #6EE7B7; color: #0A0A0F; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 700; font-size: 14px;">View full review</a>
          <a href="${prUrl}" style="background: rgba(255,255,255,0.06); color: #fff; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-size: 14px; border: 1px solid rgba(255,255,255,0.1);">View PR on GitHub</a>
        </div>
